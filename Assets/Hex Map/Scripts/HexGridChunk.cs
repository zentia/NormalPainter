...
public class HexGridChunk : MonoBehaviour
{
    ...
    {
        bool hasRoadThroughtEdge = cell.HasRoadThroughEdge(direction);
        ...
        else if (cell.IncomingRiver == cell.OutgoingRiver.Opposite())
        {
            ...
            if (previousHasRiver)
            {
                if (!hasRoadThroughtEdge && !cell.HasRoadThroughEdge(direction.Next()))
                    return;
                corner = HexMetrics.GetSecondSolidCorner(direction);
            }
            else
            {
                if (!hasRoadThroughtEdge && !cell.HasRoadThroughEdge(direction.Previous()))
                    return;
        ...
        else
        {
            ...
            if (!cell.HasRoadThroughEdge(middle)&&!cell.HasRoadThroughEdge(middle.Previous())&&!cell.HasRiverThroughEdge(middle.Next()))
                return;
            roadCenter += HexMetrics.GetSolidEdgeMiddle(middle) * 0.25f;
        }
        Vector3 mL = Vector3.Lerp(roadCenter, e.v1, interpolators.x);
        Vector3 mR = Vector3.Lerp(roadCenter, e.v5, interpolators.y);
        TriangulateRoad(roadCenter,mL,mR,e,hasRoadThroughtEdge);
        if (previousHasRiver)
        {
